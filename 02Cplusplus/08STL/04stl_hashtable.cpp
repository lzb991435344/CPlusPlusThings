...
/**
0.导语
哈希表，是作为unordered_map与unordered_set等的底层容器
...
(1) 使用范围哈希(实际上就是我们通常说的除留余数法)，不缓存hash code。
template<typename _Key, typename _Value, typename _ExtractKey,
typename _H1, typename _H2, typename _Hash>
...
(2)使用范围哈希(实际上就是我们通常说的除留余数法)，缓存hash code。
...
当发生哈希冲突的时候，该函数会返回一个不小于n的素数来作为下一个桶。



(5)素数表

怎么查找素数呢？ 发现上面有个__prime_list,于是取查找，在libstdc++v3/src/shared/hashtable-aux.cc中
找到了所有的素数表。 里面总共有256+1+49或者256+49个。 如果sizeof(unsigned long)!=8 就是256+1+49个，
否则就是256+49个。
...
到这里就很明确了，对于节点，分为包含hash code与不包含，具体是根据传递的模板参数，
来调用相应的偏特化版本。



3.迭代器
迭代器基类显示使用using的语法，这个语法类似于typedef，后面定义就可以直接使用__node_type语法来定义，
_M_incr函数完成链表下一个节点获取。
...
第一：using 的使用
(1)hashtable的迭代器属于forward_iterator
(2)重载了++,--,*,->，这四个操作符
...
内部结构为在每个元素中维护一个单链表, 然后在单链表上执行元素的插入、搜寻、删除等操作，
每个元素被称为桶(bucket)，底层构建先采用H1计算出key的hash code，再通过除留余数法H2得到其对应的桶。
...
在_Hash_code_base中有如下实现：

而_M_h1返回的是_H1，_H1不知道是什么情况下，我们可以在unordered_map中查找到是_Hash=hash<_Key>，
因此下面这个函数就是数学表达式： h1(k)来获取hash code。
