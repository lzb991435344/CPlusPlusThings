...
int main() {
	...
	//04.删除指定范围的元素
	//auto iter = vec.erase(std::begin(vec) + 1, std::begin(vec) + 3);
	//for (auto it = vec.begin(); it != vec.end(); ++it) {
	//	std::cout << *it;//1 4 5
	//}
...
array是静态的，一旦配置了就不能改变，而 vector是动态空间，随着元素的加入，它的内部机
制会自行扩充空间以容纳新元素。
...
(2) typedef typename __gnu_cxx::__alloc_traits<allocator<_Tp>>::pointer
       	pointer;

		typedef std::allocator_traits<_Alloc>           _Base_type;
		typedef typename _Base_type::pointer            pointer;

(3)typedef std::allocator_traits<_Alloc>::pointer
       	pointer;
...
_Vector_base中有一个内存管理器_Vector_impl类，该结构体继承allocator
...
public:
	...
	void _M_deallocate(pointer __p, size_t __n) {
		typedef __gnu_cxx::__alloc_traits <_Tp_alloc_type> _Tr;
		if (__p)
			_Tr::deallocate(_M_impl, __p, __n); // 最后调用allocator_traits的deallocate,而该函数又是根
			据传递进来的_M_impl进行deallocate,一直往后，最后调用的就是free函数
	}
...
_Vector_base专门负责vector的内存管理，内部类_M_impl通过继承_Tp_alloc_type(也就是allocator)得到内存分配
释放的功能，_M_allocate和_M_deallocate分别分配和释放vector所用内存，vector只需要负责元素构造和析构。
...
_M_current是指向迭代器位置的指针，这是一个随机访问型指针，operator+和operator-等移动操作可以直接
移动到目的地，非随机访问型指针只能一步步移动。
...
3.vector的数据结构
(1)vector内存由_M_impl中的M_start，_M_finish，_M_end_of_storage三个指针管理，
所有关于地址，容量大小等操作都需要用到这三个指针：
...
(2)shrink_to_fit()
_M_finish和_M_end_of_storage之间的空间没有数据，有时候这是一种浪费，c++11提供了一个很有用的函数
shrink_to_fit()，将这段未使用空间释放，主要调用了下面代码，
...
第一次swap把__first的指针与__last的指针之间的数据拷贝到__result指针所指向的起始位置。 
第二次swap获得__last的指针对应的迭代器。
...
先分配size()大小的内存空间，用于存储begin()与end()之间的数据，释放原来的vector空间，
新的vector只包含size()数量的数据，并修改_M_start与_M_end_of_storage指向。
...
5.vector
插入涉及到内存分配，动态调整，与一开始提到的vector与array区别
