/**
(1)header不仅指向root,也指向红黑树的最左节点,以便用常数时间实现begin(),并且也指向红黑树的最右边节点,
以便 set相关泛型算法（如set_union等等）可以有线性时间表现.

(2)当要删除的节点有两个子节点时，其后继节点连接到其位置，而不是被复制，因此，唯一使无效的迭代器是引
用已删除节点的迭代器。

上述话翻译成图，如下，相比于普通的红黑树多了一个header节点，并且为红色。普通的红黑树是以100节点开始
的，且满足下面五条性质：

(i)每个节点或是红色的,或是黑色的.
(ii)根节点是黑色的.
(iii)每个叶节点（NULL）是黑色的.
(iiii)如果一个节点是红色的，则它的两个孩子节点都是黑色的.
(iiiii)对每个节点，从该节点到其所有后代叶节点的简单路径上，均包含相同数目的黑色节点.
当然这里的rb_tree也是一样满足这几条性质，迭代器的begin指向红黑树根节点，也就是header的父亲，而end指
向header节点。
...
基类中包含了指向自己的指针，分别定义了left、right、parent，同时包含了一个颜色标记常量，而里面有
两个核心函数，目的是获取红黑树中最小节点与最大节点。 我们知道对于二分搜索树获取最小节点就是左子
树一直往下搜，最大节点就是右子树一直往下搜即可。
...
3.红黑树迭代器
红黑树迭代器里面有一个红黑树基类成员，然后通过该成员进行迭代器的相关操作。 同时，我们可以知
道该迭代器属于bidirectional_iterator_tag。
...
// 第二种情况:_x为header节点了,那么_x就是最后的后继节点. 简言之_x为最小节点且往上回溯,一直为父节
//点的右孩子,直到_x变为父节点,_y为其右孩子 
...
4.红黑树操作
比较重要的是，里面使用节点基类来声明了一个指针。还包含了一个_Rb_tree_impl用来对红黑树初始化操作
与内存管理操作。里面还包含了两种迭代器，一个rbtree，另一个是reverse，说明支持rbegin,rend操作。
...
在tree.cc源码中实现函数为local_Rb_tree_rotate_left与local_Rb_tree_rotate_right。 
下面我们将源码进行剖析成比较容易理解的代码，具体见注释。 大家会发现函数名与变量名与源码不同，
是因为下面是当时自己实现的，但是不影响源码阅读，就直接拿来对比了。
...
void rightRotate(Node *_x) {
	...
	if (__y && __y->_M_color == _S_red)     // case1: 叔叔节点存在，且为红色
	{
	 ...
	 //解决办法是：颜色翻转，父亲与叔叔的颜色都变为黑色,祖父节点变为红色,然后当前节点设为祖父，
	//依次网上来判断是否破坏了红黑树性质
	...
	_Rb_tree_insert_and_rebalance完整解析：
		...
		// Rebalance.
		while (__x != __root
			&& __x->_M_parent->_M_color == _S_red)   // 若新插入节点不是为RB-Tree的根节点，且其父节
			//点color属性也是红色,即违反了性质4.
		{
			...
			if (__x->_M_parent == __xpp->_M_left)   // 父亲是祖父节点的左孩子
			{
				...
				if (__y && __y->_M_color == _S_red)     // case1: 叔叔节点存在，且为红色
				{
					 ...
					 // 解决办法是：颜色翻转，父亲与叔叔的颜色都变为黑色,祖父节点变为红色,然后当前节点
					设为祖父，依次网上来判断是否破坏了红黑树性质
...
case 1:U为红色，P、N也都为红色，则可以通过改变颜色，自底向上递归调整，下次N就变味G，
往上判断即可。如果碰巧将根节点染成了红色, 可以在算法的最后强制root改为黑。
