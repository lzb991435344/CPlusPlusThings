...
(1)map的key为key,value为key+data,与set是不同的，set是key就是value，value就是key。
(2)map的key不可修改,map与multimap的插入调用函数不同，影响了其key是否对应value。
(3)initializer_list使用
(4)map有[]操作符，而multimap没有[]操作符。
...
因为它的构造函数里面，参数有个initializer_list,因此我们可以针对这个对map进行使用。 
但是map没有类似的构造，它也应用在map构造函数，insert与=处，跟上面是一样的
...
在初始化的时候，定义及赋值的时候就直接调用构造，后面再次赋值，就是先调用拷贝构造
(有可能会被编译器优化)，再调用=操作符。
...
我们思考一下，因为multimap会根据key，有可能会对应多个value，那如果我们通过[]获取对应key的value，
此时到底获取的是哪个value呢，所以在STL源码中没有重载这个操作符！
