...
template<bool _Cache>
using __umap_traits = __detail::_Hashtable_traits<_Cache, false, true>;
可以对比上述两个发现__umap_traits里面有一串__cache_default，我们再看一下模板参数为bool类型，
故可以打印出来是false还是true，经过实测，为false，表示不缓存hash code。
...
看到有三个using，理解为三个typedef，依次表示：hash code缓存与否，是否是常迭代器，是否是唯一的key，
再往上回头看，传递进来的是三个模板参数，分别是false,false,true，也验证了unordered_map是唯一的key，那么对应的unordered_multimap就是不唯一的key，最后一个参数为false。我们翻阅到相应代码如下：
...
using __uset_hashtable = _Hashtable<_Value, _Value, _Alloc,
				...
				__detail::_Default_ranged_hash,
				__detail::_Prime_rehash_policy, _Tr>;
可以看到传递给_Hashtable_traits的是false,true,true。对于unordered_set来说使用的是const iterator
与唯一的key,我们再看一下unordered_multiset：
...
结论2：unordered_map与unordered_multimap采用的迭代器是iterator，而unordered_set与
	unordered_multiset采用的迭代器是const_iterator。

结论3：unordered_map与unordered_multimap的key是key，value是key+value；而unordered_set
	与unordered_multiset的key是Value，Value也是Key。
...
int main() {
	...
	//底层为hash
	std::unordered_set<int, std::equal_to<int>, std::allocator<int> > unset;
	unset.insert(1);
	unset.insert(3);
