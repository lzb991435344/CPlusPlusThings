/**
STL之set与multiset那些事
set / multiset以rb_tree为底层结构，因此有元素自动排序特性。排序的依据是key，而set / multiset元素
的value和key合二为一：value就是key。

我们无法使用set / multiset的iterators改变元素值(因为key有其严谨排列规则)。 set / multiset的iterator是
其底部RB tree的const - iterator，就是为了禁止用户对元素赋值。

set元素的key必须独一无二，因此其insert使用的是rb_tree的_M_insert_unique()，而multiset元素的key可以重
复，因此其insert使用的是rb_tree的_M_insert_equal()。
...
具体代码再第二个问题中会有，这里给出我们通常写代码后内部逻辑，我们看到里面有个红黑树，而红黑树的
定义key与value是一样的，所以回答了这个问题。(源码中typedef都是来自key)。
...
经过前面分析，让我们想起了queue、priority_queue、stack，他们都使用的是底层的容器，所以称
为容器适配器，而set也是使用底层的容器，所以也可以被称为container adapter, 即容器适配器。
...
我们来简单看一下这个函数实现： 下面_M_get_insert_unique_pos返回的是个pair，如果插入的key相同
则pair的second为0，根据是否为0可以判断是否key重复，在下面代码中判断时候，当second不为0，也就
是不重复的时候，那么就可以直接插入，此时直接构造一个second为true的pair, 否则构造一
个second为false的pair。
...
我们对比multiset与set的这几个函数发现，返回的pair有着显著的差异，之前的set需要返回
最终是否插入成功，因为key不可重复，而multiset不需要返回是否插入成功，所以pair中不存
在bool类型，故它是直接返回的插入点所构成的pair，因此，与之前相比较而言，不管你有多
少个key，重复如何，都可以插入进去。
...
int main() {
	...
	//auto it = ms.begin();
	//for (; it != ms.end(); ++it) {
	//	std::cout << *it << std::endl; //1 2 2
	//}

	std::unordered_set<int> ums;
	ums.insert(5);
	ums.insert(3);
	ums.insert(6);
	...
	std::unordered_set<std::string> third({ "pink", "orange","yellow" }); // init list
	...
	//sixth contains: pink blue orange yellow green red
	return 0;
}*/
